...
export default function RoomSelection() {
  ...
  const handleJoinRoom = async () => {
    if (!roomSlug.trim()) { setError('Please enter or generate a room slug'); return; }
    setLoading(true); setError('');
    try {
      const token = localStorage.getItem('token');
      if (!token) { setError('Please sign in first'); navigate('/signin'); return; }
      const response = await axios.get(`${backendUrl}/rooms/${roomSlug.trim()}`);
      const roomId = response.data.room.id;
      if (!roomId) { setError('Room not found. Please check the room slug.'); setLoading(false); return; }
      navigate(`/canvas/${roomId}`);
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to join room. Please check the room slug.');
    } finally { setLoading(false); }
  };

  const handleCreateAndJoin = async () => {
    if (!generatedSlug) { generateRoomSlug(); return; }
    setLoading(true); setError('');
    try {
      const token = localStorage.getItem('token');
      if (!token) { setError('Please sign in first'); navigate('/signin'); return; }
      const response = await axios.post(`${backendUrl}/room`, { name: generatedSlug }, { headers: { token } });
      const roomId = response.data.room.id;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 flex items-center justify-center px-4 sm:px-6 py-8 sm:py-12">
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-5 sm:p-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-2 text-center">Choose Your Canvas</h1>
          <p className="text-slate-600 text-center mb-6 sm:mb-8 text-sm sm:text-base">Create a new room or join an existing one</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="border-2 border-slate-200 rounded-xl p-4 sm:p-6 hover:border-blue-500 transition-colors">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h2 className="text-lg sm:text-xl font-semibold text-slate-800">Create New Room</h2>
              </div>
              <p className="text-slate-600 text-xs sm:text-sm mb-4">Generate a unique room slug and share it with others</p>

              {generatedSlug && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                  <p className="text-xs text-slate-600 mb-1">Your Room Slug:</p>
                  <div className="flex items-center gap-2">
                    <code className="text-xs sm:text-sm font-mono text-blue-700 flex-1 break-all">{generatedSlug}</code>
                    <button onClick={copyToClipboard} className="p-2 hover:bg-blue-100 rounded transition-colors flex-shrink-0" title="Copy">
            ...
            <div className="border-2 border-slate-200 rounded-xl p-4 sm:p-6 hover:border-blue-500 transition-colors">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h2 className="text-lg sm:text-xl font-semibold text-slate-800">Join Existing Room</h2>
              </div>
              <p className="text-slate-600 text-xs sm:text-sm mb-4">Enter a room slug to join a collaborative canvas</p>
